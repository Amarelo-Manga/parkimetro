...
class add_endereco{
    protected static $metabox_config = array(
        'endereco', // Metabox Name
        'Endereço', // Metabox Title
        'rede', // PostType do Metabox
        'normal',
        'high'
    );
    ...
    static function metabox(){
        ...
        if(!isset($metadata['endereco'])){ //Name do Metabox
           $content = "";
        }else{
            $content = $metadata['endereco'][0]; //Name do Metabox
        };
    ?>  

    <!-- Tipos de Campos-->
    <input type="text" name="<?php echo __CLASS__ ?>[endereco]" value="<?php echo  $content; ?>" style="background-color: #fff; font-size: 1.7em; height: 1.7em; line-height: 100%; margin: 0; outline: 0; padding: 3px 8px; width: 100%;" />
